  ...
  async purchaseSubscription(productId: string, userId?: string) {
    try {
      ...
      if (!request.ios && !request.android) {
        throw new Error(`Unsupported platform for IAP purchase: ${Platform.OS}`);
      }
    ...
    try {
      ...
      if (Platform.OS === 'ios') {
        ...
        if (!purchase.purchaseToken) {
          throw new Error('Missing purchase token for Android purchase verification');
        }
      ...
      const response = await axios.post(`${apiBaseUrl}/api/iap/complete`, payload);
