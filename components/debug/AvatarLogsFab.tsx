...
const AvatarLogsFab: React.FC = () => {
	...
	const handleCopy = useCallback(async () => {
		const payload =
			avatarLogs.length > 0
				? avatarLogs.map((entry) => formatEntry(entry)).join("\n")
				: "No avatar logs captured yet.";
		try {
			await Clipboard.setStringAsync(payload);
			Alert.alert("Logs copiés", "Les logs avatar ont été copiés.");
		} catch {
	...
	return (
		<>
			<Pressable
				...
				onPress={openLogs}
			>
				<Text style={styles.floatingButtonLabel}>Logs avatar</Text>
			</Pressable>
			...
			>
				<BottomSheetView style={styles.sheetContainer}>
					...
					<View style={styles.headerRow}>
						<Text style={styles.sheetTitle}>Logs avatar</Text>
						<Text style={styles.logCount}>{avatarLogs.length} entrées</Text>
					</View>

					<View style={styles.actionsRow}>
						<TouchableOpacity style={styles.secondaryButton} onPress={closeLogs}>
					...
					{avatarLogs.length === 0 ? (
						<View style={styles.emptyState}>
							<Text style={styles.emptyText}>Aucun log avatar capturé.</Text>
						</View>
					) : (
